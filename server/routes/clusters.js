...
router.get('/project/:projectId', auth, async (req, res) => {
  const { rows } = await db.query(
    `SELECT c.*, COUNT(t.id) as task_count FROM clusters c
     ...
     GROUP BY c.id ORDER BY c.created_at DESC`, [req.params.projectId]);
  res.json(rows);
});

router.get('/:id', auth, async (req, res) => {
  const { rows } = await db.query('SELECT * FROM clusters WHERE id=$1', [req.params.id]);
  if (!rows[0]) return res.status(404).json({ error: 'Not found' });
  const tasks = await db.query(
    `SELECT t.*, m.name as assignee_name FROM tasks t LEFT JOIN members m ON t.assignee_id=m.id
     WHERE t.cluster_id=$1`, [req.params.id]);
  const reviews = await db.query(
    `SELECT cr.*, m.name as reviewer_name FROM cluster_reviews cr
     JOIN members m ON cr.reviewer_id=m.id WHERE cr.cluster_id=$1 ORDER BY cr.created_at DESC`, [req.params.id]);
...
router.post('/:id/submit-review', auth, managerOnly, async (req, res) => {
  const { rows } = await db.query(
    "UPDATE clusters SET status='in_review' WHERE id=$1 RETURNING *", [req.params.id]);
  res.json(rows[0]);
});
...
router.post('/:id/review', auth, managerOnly, async (req, res) => {
  ...
  try {
    ...
    if (decision === 'approved') {
      await client.query("UPDATE clusters SET status='approved' WHERE id=$1", [req.params.id]);
    } else {
      await client.query(
        "UPDATE clusters SET status='needs_rework', rework_count=rework_count+1 WHERE id=$1", [req.params.id]);
      if (rework_task_ids?.length) {
        await client.query(
          "UPDATE tasks SET stage='Todo',updated_at=NOW() WHERE id=ANY($1::uuid[])", [rework_task_ids]);
      }
    }
...
router.patch('/:id/complete', auth, managerOnly, async (req, res) => {
  const { rows } = await db.query(
    "UPDATE clusters SET status='completed' WHERE id=$1 RETURNING *", [req.params.id]);
  res.json(rows[0]);
});
