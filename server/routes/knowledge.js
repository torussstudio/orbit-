...
router.get('/project/:projectId', auth, async (req, res) => {
  const folders = await db.query(
    'SELECT * FROM knowledge_folders WHERE project_id=$1 ORDER BY created_at', [req.params.projectId]);
  const files = await db.query(
    `SELECT kf.*, m.name as uploaded_by_name FROM knowledge_files kf
     LEFT JOIN members m ON kf.uploaded_by=m.id WHERE kf.project_id=$1 ORDER BY kf.created_at`, [req.params.projectId]);
  const notes = await db.query(
    `SELECT kn.*, m.name as created_by_name FROM knowledge_notes kn
     LEFT JOIN members m ON kn.created_by=m.id WHERE kn.project_id=$1 ORDER BY kn.created_at`, [req.params.projectId]);
...
router.post('/files/upload', auth, upload.single('file'), async (req, res) => {
  ...
  try {
    ...
    const { rows } = await db.query(
      ...
      [folder_id||null, project_id, file.originalname, fileUrl, file.size, file.mimetype, req.user.id]);
    res.status(201).json(rows[0]);
  } catch (e) {
