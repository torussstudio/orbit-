    ...
    <>
      <div className="page-header" style={{ paddingBottom: '16px' }}>
        <div>
          ...
          {project.description && <div className="page-subtitle" style={{ marginTop: '4px', maxWidth: '600px' }}>{project.description}</div>}
        </div>
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <span className={`badge badge-${project.status}`}>{project.status?.replace('_', ' ')}</span>
          <div style={{ display: 'flex', gap: '6px' }}>
            {project.members?.map(m => (
              <div key={m.id} className="user-avatar" title={m.name} style={{ width: '28px', height: '28px', fontSize: '11px' }}>{m.name[0]}</div>
            ))}
          </div>
        </div>
      </div>
