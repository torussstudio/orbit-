...
// // Groups a task list into { parentId: [subtasks] }. Used by both the
// // board and the list view so sub-tasks render nested under their
// // parent's card/row instead of appearing as their own top-level item.
...
//   // Only main tasks count toward the header badge and drive the
//   // board/list rows — sub-tasks are nested inside their parent instead
//   // of being counted/rendered as their own item.
//   const mainTaskCount = filteredTasks.filter((t) => !t.parent_task_id).length;
...
//             }}
//           >
//             {mainTaskCount} task{mainTaskCount !== 1 ? "s" : ""}
//           </span>
...
//   // Only main tasks (no parent_task_id) get their own column position —
//   // sub-tasks are looked up per parent and rendered nested inside that
//   // parent's card, regardless of the sub-task's own stage.
...
//   // its dropped position, then persist that whole order in one call.
//   const moveTask = async (taskId, targetStage, targetIndex) => {
//     const task = mainTasks.find((t) => String(t.id) === String(taskId));
//     if (!task) return;
//     const managerOnly = ["Done"];
//     if (user.role === "member" && managerOnly.includes(targetStage)) return;

//     const destList = mainTasks
//       .filter((t) => t.stage === targetStage && String(t.id) !== String(taskId))
//       .sort((a, b) => (a.sort_order ?? 0) - (b.sort_order ?? 0));
...
//       >
//         {stages.map((stage) => {
//           const stageTasks = mainTasks.filter((t) => t.stage === stage);
//           return (
//             <div
//               key={stage}
//               style={{
...
//                       style={{
//                         fontSize: "9px",
//                         color: stOverdue ? "var(--danger)" : "var(--text-3)",
//                       }}
//                     >
//                       {stOverdue ? "⚠ " : ""}
...
//                     <button
//                       className="btn btn-ghost btn-sm"
//                       style={{ fontSize: "10px", padding: "1px 6px", color: "var(--danger)" }}
//                       onClick={(e) => {
//                         e.stopPropagation();
//                         onDelete(st.id);
//                       }}
...
//                           <td
//                             style={{
//                               color: stOverdue ? "var(--danger)" : "var(--text-2)",
//                               fontSize: "12px",
//                             }}
//                           >
//                             {formatDate(st.due_date)}
