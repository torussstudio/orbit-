...
export default function TaskDetail() {
  ...
  const [reworkConfirm, setReworkConfirm] = useState({
    show: false,
    subtask: null,
  });
          ...
          <div className="breadcrumb td-breadcrumb">
            ...
            {task.parent_task_id ? (
              isManager ? (
                <>
                  <Link
                    to={`/projects/${projectId}/tasks/${task.parent_task_id}`}
                  >
                    Task
                  </Link>
                  <span className="breadcrumb-sep">/</span>
      ...
      <div className="page-body td-main-grid">
        <div>
          {/* Status - read-only, main tasks only. Derived automatically
              from subtask stages (see backend recompute logic); moved to
              top since it's now informational rather than an action. */}
          {!task.parent_task_id && (
            <div className="card" style={{ marginBottom: "16px" }}>
              ...
              >
                Automatically determined by the sub tasks' progress — change
                stages on the sub tasks below to update this.
              </p>
              <div style={{ display: "flex", gap: "8px", flexWrap: "wrap" }}>
                {stages.map((s) => {
                  const active = task.stage === s;
                  return (
                    <span
                      key={s}
                      className={`td-status-pill ${active ? "active" : ""}`}
                      style={
                        active
                          ? {
                              background:
                                STAGE_DOT_COLORS[s] || "var(--accent)",
                            }
                          : {}
                      }
                    >
                ...
                >
                  {task.subtasks?.map((st) => (
                    <div
                      ...
                      style={{
                        borderLeft: `3px solid ${PRIORITY_COLORS[st.priority]}`,
                      }}
                    >
                              ...
                              >
                                {(isManager
                                  ...
                                  <div
                                    key={s}
                                    onClick={() =>
                                      handleSubTaskStageSelect(st, s)
                                    }
                                    style={{
                                      padding: "7px 12px",
                                      borderRadius: "6px",
                                      cursor:
                                        s === st.stage ? "default" : "pointer",
                                      fontSize: "12px",
                                      fontWeight: s === st.stage ? 700 : 400,
                                      color:
                                        s === st.stage
                                          ? "var(--accent)"
                                          : "var(--text)",
          ...
          {/* Stage changer - shown inside subtasks for everyone (only
              section on this page where stage is still manually editable) */}
          {task.parent_task_id && (
            <div className="card" style={{ marginBottom: "16px" }}>
              <h3
                style={{
            ...
            {[
              ...
              ...(task.subtasks?.some((s) => s.time_taken)
                ? [
                    [
                      "Total Time",
                      `⏱ ${task.subtasks.reduce((sum, s) => sum + (s.time_taken || 0), 0)} min`,
                    ],
                  ]
                : []),
            ].map(([l, v]) => (
      ...
      {timeTakenModal.show && (
        ...
        >
          ...
          >
            ...
            <strong>In Review</strong>. How long did this subtask take?
          </p>
        ...
        >
          ...
          <div className="td-review-options">
            <div
              ...
              style={{
                ...
                padding: "16px",
                borderRadius: "8px",
                border: `2px solid ${reviewAction === "done" ? "var(--success)" : "var(--border)"}`,
                cursor: "pointer",
                textAlign: "center",
                background:
                  reviewAction === "done"
            ...
            >
              ...
              <div
                style={{
                  ...
                  fontWeight: 600,
                  color:
                    reviewAction === "done" ? "var(--success)" : "var(--text)",
                }}
              >
                Mark as Done
              </div>
            ...
            <div
              ...
              style={{
                ...
                padding: "16px",
                borderRadius: "8px",
                border: `2px solid ${reviewAction === "rework" ? "var(--danger)" : "var(--border)"}`,
                cursor: "pointer",
                textAlign: "center",
                background:
                  reviewAction === "rework"
            ...
            >
              ...
              <div
                style={{
                  ...
                  fontWeight: 600,
                  color:
                    reviewAction === "rework" ? "var(--danger)" : "var(--text)",
                }}
              >
                Send for Rework
              </div>
          ...
          {reviewAction === "rework" && (
            ...
            >
              ...
              >
                Set a new due date for the rework cycle
              </div>
            </div>
          )}
          <div className="modal-actions">
